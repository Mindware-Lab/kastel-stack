# Kastel Stack - Sketch Business Plan (v0.2)
...
Designed for the LLM era: the model can propose; **policies decide**. Workflows are only "banked" after **dry-run -> rollback test -> delayed re-check**.

**Differentiator (1): Tiered sovereignty + exit-by-design.**  
Including an **ownable appliance tier** where customers can stop paying and still keep operating (optional maintenance/support). This supports digital sovereignty via **anti-chokepoint architecture** (no single platform dependency for core ops).

**Differentiator (2): G-Loop Proof Engine (Trident-G lineage).**  
A built-in learning loop that **proves what works** and only promotes automation after portability checks. In ops terms:  
- **EOU loop** (Execute-Observe-Update) against mission success signals  
- **Wrapper/stake swaps** to test portability under changed conditions  
- **Bank only what holds** (validated scripts, not brittle automations)

This creates "far transfer" for operations: improvements generalise across contexts rather than overfitting one tool/workflow.

**Go-to-market:** implement inside HRPLab first (proof-producing case study), then sell to a narrow initial niche via founder-led pilots. Once stable, add a marketing operator (e.g., Sydney startup circles) to scale distribution.
...
### 3.2 G-Loop Proof Engine (learning + proof as a product primitive)
Kastel Stack includes an internal "G-Loop" that turns ops into **controlled learning**:
- **Execute-Observe-Update (EOU):** each workflow run emits an outcome signal (e.g., response latency, conversion, refund rate, churn proxy).
- **Portability checks:** changes must survive **wrapper swaps** (different channel/tool) and **stake swaps** (higher load/pressure) before promotion.
- **Banking rule:** only changes that survive checks become default "auto-run" scripts; otherwise they stay draft-for-approval or escalate.

This makes Kastel Stack meaningfully different from "connectors + AI": it is **automation that improves with proof**, not assumption.

### 3.3 Tiered sovereignty (what customers buy)
...
### 3.4 Finance posture (trust-critical)
...
- Publish 2-3 proof assets:
  ...
  - **G-Loop proof fragment** (what changed, what check was used, what got banked)
...
- G-Loop becomes a productised learning feature (banked scripts library, portability evidence)
...
- Bank workflows only after proof checks (G-Loop discipline)
...
- explicit ban on autonomous money movement (Finance Intents + approvals)
...
- **G-Loop Proof Engine**: controlled learning + portability checks (ops "far transfer")
...
Kastel Stack packages the "safe way to automate" and the "safe way to self-host" **plus a proof engine that improves reliability over time**.
...
(See `01_business/model/kastel_stack_speculative_financials_updated.xlsx` for the speculative monthly model.)
...
- Additional engineering (connector library + reliability tooling; G-Loop enhancements)
...
5) **Competition from incumbents**  
   -> Mitigate by owning the trust + sovereignty niche (exit-by-design, local-first tiers, audit-first posture) and the G-Loop proof engine.
...
- a first demonstration of the **G-Loop "bank only what holds"** mechanism in ops context
...
- expand the G-Loop proof engine (portable "banked scripts" library and checks)